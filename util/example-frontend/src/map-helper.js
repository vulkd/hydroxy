...
import { getBbox } from '../threegeo/geo-utils';
ThreeGeo.getBbox = getBbox
...
class MapHelper {
    constructor(options={}) {
        ...
        this.onMapButtonZoomClick = actual.onMapButtonZoomClick;
        ...
        document.querySelector('.leaflet-control-zoom-in').addEventListener('click', () => {
            this.onMapButtonZoomClick(this.map.getZoom())
        })
    ...
    plotCam(cam) {
        let lineCam = MapHelper.mkLineCam(cam, this.origin, this.projection.unitsPerMeter, this.map.getZoom());
